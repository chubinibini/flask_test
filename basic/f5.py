...
app = Flask(__name__)
# 세션을 위해서 시크릿키 지정
app.secret_key = 'fdhfsiwe' #임의값, 통상 해시값 활용

# 로그인을 하여 세션을 얻은후 홈페이지를 진행해야 사이트의 내용을 보여주겠다 =>컨셉
@app.route('/')
def home():
    if not 'uid' in session: # 세션안에 uid값이 존재하는가?
        #return redirect('/login') #url을 사용할때는 하드코딩하지않는다
        #url_for('사용하고자하는 url과 연결된 함수명을 기입')
...
def login():
    ...
    if request.method == 'GET':
        ...
        if result: # 3. 회원이면
            ...
            #        클라이언트가 간편하게 웹을 이용할수있게 도움줌
            #        단점:접속유저가 많으면 서버측 메모리에 부하가온다 -> 대체제/대안필요
            #        JMT를 사용하여 보완(사이트구성시 인증쪽에서 활용:차주진행)
            ...
            pass
        #return redirect('https://www.naver.com') # 요청을 다른 URL로 포워딩함
